...
- 新增用户侧 skill 安装目录与 lockfile 模型，用于统一管理已安装 skill
- 新增 `hs-cli skills install|remove|link|doctor` 命令，建立“安装到用户环境并接入 Agent”的主路径
...
- `ai-skills-authoring`: skill 内容创建与质量校验能力
- `ai-skills-distribution`: skill 安装、链接、卸载与环境检查能力

### Modified Capabilities

- CLI 命令集新增 `skills install/remove/link/doctor` 子命令
- 现有 `skills` 体系从“读取型工具接口优先”调整为“安装型用户路径优先”
