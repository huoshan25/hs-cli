...
export function openspecCommand(program: Command): void {
  program
    .command('openspec')
    .description('OpenSpec 可视化面板')
    ...
    .action(async (options: { doc?: string; theme?: string; ui?: string; watch?: boolean }) => {
      ...
      try {
        ...
        if (ui === 'web') {
          ...
          try {
            ...
            console.error(`OpenSpec 面板启动失败: ${fallbackError.message}`);
            process.exit(1);
          }
        }
        console.error(`OpenSpec 面板启动失败: ${error.message}`);
        process.exit(1);
      }
    });
}
